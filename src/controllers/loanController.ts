...
export const createLoan = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const targetUserId = (req.user.role === 'ADMIN' && validatedData.userId)
            ? validatedData.userId 
            : req.user.userId;

        const loan = await loanService.createLoan(targetUserId, validatedData.bookId);
        ...
        if (error.message === '# Користувач незнайдений' || error.message === '# Книга незнайдена') {
            return res.status(404).json({ error: error.message });
        }
        
        if (error.message === '# Книга недоступна') {
...
export const returnLoan = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (error.message === '# Позика незнайдена') {
            return res.status(404).json({ error: error.message });
        }
        
        if (error.message === '# Позика вже повернута') {
            return res.status(400).json({ error: error.message });
        }
